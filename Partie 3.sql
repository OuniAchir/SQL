...
create user admin identified by psw
...
--user_sys_privs contient tous les privilèges systèmes de l'utilisateur connecté
---4.	Donner les privilèges suivants à Admin: créer des tables, des utilisateurs. Vérifier.
connect dbaintervention/psw
grant create user, create table to admin;
--vérification
connect admin/psw
select privilege, admin_option from user_sys_privs;
...
connect system/psw
 ...
 9--Créer un index NOMEMP_IX sur l’attribut NOMEMP de la table EMPLOYE. Que remarquez-vous ?
 connect admin/psw
...
connect dbaintervention/psw
--privilège système
revoke create session, create user, create table from admin;
--vérification
connect system/psw
